"""aggregation functions and trigger
...
def upgrade() -> None:
    op.execute(
        ...
        BEGIN
            SELECT ta.user_id, t.lesson_id, ta.is_correct
            INTO v_user_id, v_lesson_id, v_is_correct
    ...
    op.execute(
        ...
        BEGIN
            ...
            FROM (
                ...
                FROM task_attempt ta
                JOIN task t ON t.id = ta.task_id
                WHERE ta.user_id = v_user_id AND t.lesson_id = v_lesson_id
    ...
    op.execute(
        r'''
        CREATE OR REPLACE FUNCTION apply_all_aggregations_on_attempt()
...
def downgrade() -> None:
    ...
    op.execute('DROP FUNCTION IF EXISTS apply_all_aggregations_on_attempt();')
